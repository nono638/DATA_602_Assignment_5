...
try:
    ...
    print('something went wrong importing...')
...
def exercise01():
    ...
    print("Exercise 1.")
    df = pd.DataFrame({1:[1,5,7], 2:[33,44,55], 3:[-1,-1,-1], 4:[7,6,5]})
...
def exercise02(a):
    ...
    # ------ Place code below here \/ \/ \/ ------
    #This shouldn't return a variable called arrary, you shouldn't name variables the same things as a type of object...
...
def exercise07(sd,m,s):
    ...
    # ------ Place code below here \/ \/ \/ ------
    random_numbers = numpy.random.normal(m, sd, s)
...
def exercise08():
    ...
    row_count = len(df)
    avg_sq_ft = df['sq__ft'].mean() #average is mean; right?  Could be median for this particular example
...
def exercise12(n):
    ...
    while(counter < n*2):
        if (OnesFirst):
            testList.append( [1,0] *2 *n )
            OnesFirst = False
        else:
            testList.append( [0, 1] * 2 * n )
            OnesFirst = True
        counter +=1

    for i in range(len(testList)):
        testList[i] = tuple(testList[i])
    #print('testList:')
    testList = tuple(testList)
    #print(testList)
    checkerboard_matrix = np.array(testList)
    ...
    #honestly, I'm clueless as to how this could be accomplished with one line of coe
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    s = pd.Series(range(0,n), index= pd.date_range('1/1/2010-00-00-00', periods=n, freq= 'D', normalize= True, tz='Asia/Tokyo'))
    print(s)
    #plot the cumulative sum on a  chart
    #s.cumsum().plot()
    print(s.sort_index().cumsum().plot() )
...
def exercise14(words):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df = pd.DataFrame({'words':words, 'lengths':[len(x) for x in words]})
...
def exercise15():
    ...
    df = pd.read_csv('https://tinyurl.com/y63q7okz')
    df = df.iloc[::5,[0,2] ] #0 is the street column, 2 is zip
